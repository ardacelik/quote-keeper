...
router.get("/", auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select("-password"); // by using select() we won't be returning the password with the request
    res.json(user);
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // If everthing is good so far, get the user's ID
      const payload = {
        user: {
          id: user.id
        }
